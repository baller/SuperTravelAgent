...
> **生产就绪、模块化、智能的多智能体协作框架，用于解决复杂问题**

Sage 是一个先进的多智能体系统，通过无缝的智能体协作智能地将复杂任务分解为可管理的子任务。基于企业级可靠性和可扩展性设计，提供**深度研究模式**进行全面分析和**快速执行模式**进行快速任务完成。

## ✨ 核心亮点

🧠 **智能任务分解** - 自动将复杂问题分解为带依赖关系跟踪的可管理子任务  
🔄 **智能体协调** - 专业智能体间的无缝协作，具备强大的错误处理能力  
🛠️ **可扩展工具系统** - 基于插件架构，支持 MCP 服务器和自动发现  
⚡ **双重执行模式** - 根据需求在深入分析和快速执行之间选择  
🌐 **交互式网页界面** - 基于 Streamlit 的美观界面，支持实时流式可视化  
📊 **高级Token跟踪** - 全面的使用统计和所有智能体的成本监控  
⚙️ **丰富配置** - 环境变量、配置文件、命令行选项和运行时更新  
🔧 **开发者友好** - 清洁的 API、全面的文档、示例和广泛的错误处理  
...
### ✅ 官方测试模型
- **🔥 DeepSeek-V3** - `deepseek-chat` - 在复杂推理方面表现出色
- **🌟 Qwen-3** - `qwen-turbo`, `qwen-plus` - 中英文能力卓越
- **🧠 GPT-4.1** - `gpt-4-turbo`, `gpt-4o` - 所有任务的顶级性能
- **⚡ Claude-3.5 Sonnet** - `claude-3-5-sonnet-20241022` - 卓越的推理能力

### 🌐 兼容的服务商
- **OpenAI** - 直接 API 集成
- **OpenRouter** - 访问 200+ 模型
- **Anthropic** - Claude 系列模型
- **Google AI** - Gemini 系列
- **DeepSeek** - 原生 API 支持
- **阿里云** - 通义千问系列
- **Mistral AI** - 所有 Mistral 模型

> 💡 **注意**: 虽然 Sage 针对上述模型进行了优化，但它设计为可与任何 OpenAI 兼容的 API 端点协作。
...
# 执行带全面跟踪的任务
messages = [{"role": "user", "content": "分析人工智能的当前趋势并提供可行的见解"}]
